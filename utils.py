...
def in_range(dar):
	if len(dar) > 2  or len(dar) == 0: return False
...
def dartboard_heatmap(shots, title='Heatmap'):
    ...
    z_values = []
    for point in [20, 5, 12, 9, 14, 11, 8, 16, 7, 19
             , 3, 17, 2, 15, 10, 6, 13, 4, 18, 1]:
        bm = ('BE' == shots).sum() + ('be' == shots).sum() + ('Be' == shots).sum() + ('bE' == shots).sum()
        bo = ('25' == shots).sum()
        qp = (f'{point}' == shots).sum()
        dp = (f'D{point}' == shots).sum() + (f'd{point}' == shots).sum()
        tp = (f'T{point}' == shots).sum() + (f't{point}' == shots).sum()
        z_field = [bm, bo, qp, qp, qp, qp, qp, qp, qp, qp, qp, tp, qp, qp, qp, qp, qp, qp, qp, dp]
